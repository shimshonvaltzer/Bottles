...
(function (global) {
  ...
  // set per-level by generateLevel via setCapacity() before any board is
  // built, scrambled or solved. All pure-logic functions below (canPour,
  // pour, isSolved, boardDifficulty, getReverseMoves, ...) close over this
  // module-level variable, so it must be correct BEFORE those are called;
  // JS is single-threaded and generateLevel never yields mid-build, so this
  // is safe. Default 4 keeps every existing direct call (tests, tools)
  // working without having to call setCapacity first.
  var CAPACITY = 4;
  // 12-color palette runs out at capacity 4 well before the board is
  // information-saturated (see CAPACITY_RAMP below): once boards can't be
  // made harder by mixing more, the only way left to raise the ceiling is
  // more liquid per bottle. Bottle COUNT is still capped at 14 throughout.
  function setCapacity(n) {
    CAPACITY = n;
    if (global.WaterSort) global.WaterSort.CAPACITY = n;
  }
  // Bottle capacity ramps up in steps as level rises. Below the first
  // threshold, capacity-4 boards searched hard (see generateLevel) reach
  // the low-to-mid twenties in optimal moves; measurement showed that
  // ceiling does not move with level or with scramble strength beyond that
  // point (board information capacity, not scramble weakness, is what
  // plateaus - see minSolverMoves comment: 12 colors is the palette's cap
  // and bottle count is capped at 14, so the only remaining lever is more
  // liquid per bottle). Each step measured to raise the achievable optimal
  // length by a few moves (scratchpad/tune_caps.js): ~24 at capacity 4,
  // ~25-28 at 5, ~26-30 at 6, ~28-32 at 7.
  var CAPACITY_RAMP = [
    { level: 0, capacity: 4 },
    { level: 20, capacity: 5 },
    { level: 40, capacity: 6 },
  ...
  function computeCapacity(level) {
    var cap = CAPACITY_RAMP[0].capacity;
    for (var i = 0; i < CAPACITY_RAMP.length; i++) {
      if (level >= CAPACITY_RAMP[i].level) cap = CAPACITY_RAMP[i].capacity;
    }
    return cap;
  }
  ...
  // Minimum acceptable OPTIMAL (or budget-exceeded-so-presumed-hard)
  // solution length for a level. Measurement (see scratchpad verification
  // scripts) showed that with capacity 4, best-of-N search over random
  // reverse-pour scrambles plateaus around the low-to-mid twenties
  // regardless of level or scramble strength - that is the board's
  // information capacity (12 colors x 4 units, <=14 bottles), not a weak
  // scramble. The floor therefore climbs smoothly through that range for
  // capacity-4 levels, and continues climbing past it once capacity ramps
  // to 5 (see computeCapacity / CAPACITY_RAMP_LEVEL), which raises the
  // achievable ceiling further.
  function minSolverMoves(level, numColors) {
    // Hard floor of 7 regardless of level/color count - see HARD
    // CONSTRAINTS in the generation spec ("never under 7 moves"), which
    // matters most at very low levels/small color counts.
    var target;
    if (level < 30) {
      target = 6 + Math.floor(level * 0.9); // climbs toward ~24 at capacity 4
    } else {
      // Past the capacity ramp start: keep climbing more slowly, matched to
      // what each capacity step was measured to achieve (see
      // scratchpad/tune_caps.js: ~24@cap4, ~25-28@cap5, ~26-30@cap6,
      // ~28-32@cap7). This is a soft target used to decide when the
      // candidate search can stop early (generateLevel always keeps the
      // best board found either way), so it is fine if actual boards land
      // a little below it.
      target = 24 + Math.floor((level - 30) * 0.3);
    }
    return Math.max(7, Math.min(target, 40));
  }

  // Reverse-pour based generator: start solved, undo biased reverse pours,
  // then SEARCH for a hard board rather than accepting the first one that
  // clears a bar. A single scramble attempt does not reliably find a hard
  // board (variance is high - see measurement scripts), and running the
  // exact BFS solver on every candidate is too slow to search widely
  // (measured ~100-500ms per capacity-5 board at a useful node budget).
  //
  // So generation runs in two cheap-then-precise phases:
  //   1) Generate a LARGE number of candidates (scramble + boardDifficulty
  //      only - no BFS, ~0.3-1ms each even at capacity 5) and keep the one
  //      with the highest fragmentation among structurally-valid boards.
  //      Fragmentation is a fast, well-correlated proxy for optimal solver
  //      length (verified empirically: rank-correlation is strong across
  //      both capacities - see scratchpad/corr_test.js), so "most
  //      fragmented of N" reliably approximates "hardest of N" without ever
  //      running the expensive solver N times.
  //   2) Run the real (bounded) BFS solver ONCE on that winning candidate,
  //      to (a) get the precise/measured optimal length used for the
  //      minSolverMoves bar and for reporting, and (b) as a fallback: if it
  //      somehow falls short of the bar, retry with the next-best
  //      candidates (by fragmentation) a few times, bounded by the overall
  //      wall-clock budget.
  var GEN_TIME_BUDGET_MS = 110; // soft wall-clock budget; leaves headroom under the ~500ms hard cap

  function generateLevel(level) {
    var numColors = computeNumColors(level);
    var numEmpty = computeNumEmpty(level, numColors);
    var capacity = computeCapacity(level);
    setCapacity(capacity);

    var shuffleCount = Math.min(40 + level * 10, 700);
    ...
    // BFS node budget for the real solver calls. Higher capacity means a
    // costlier per-node expansion (more possible partial-pour depths), so
    // the budget shrinks a little as capacity grows to keep each call fast
    // (measured a few ms to ~100ms per call across capacities 4-7 - see
    // scratchpad/tune_caps.js).
    var solverBudget = Math.max(2000, 15000 - (capacity - 4) * 3500);

    // N (cheap candidates) scales with level: harder targets are rarer
    // among random scrambles at low levels too, but the cheap phase is
    // fast enough to search widely everywhere.
    var cheapN = Math.min(30 + level * 3, 220);

    var startTime = Date.now();
    var candidates = []; // { bottles, fragmentation }

    for (var i = 0; i < cheapN; i++) {
      var seed = ((level * 2654435761 + i * 40503) % 2147483647 + 2147483647) % 2147483647 + 12345;
      ...
      candidates.push({ bottles: bottles, fragmentation: diff.fragmentation, structOk: structOk });
    }

    // Prefer structurally-valid candidates; among those (or, failing that,
    // among all candidates as a last resort), sort by fragmentation desc.
    var pool = candidates.filter(function (c) { return c.structOk; });
    if (pool.length === 0) pool = candidates;
    pool.sort(function (a, b) { return b.fragmentation - a.fragmentation; });

    // Evaluate only the top few by fragmentation with the real solver: the
    // fragmentation ranking already does most of the work (see corr_test.js
    // above), so trying dozens more rarely beats the first handful and
    // would otherwise burn the whole time budget chasing a bar that may be
    // above what this board size can achieve at this capacity (the bar is
    // a soft target, not a hard requirement - see minSolverMoves).
    var maxSolverCalls = capacity >= 7 ? 2 : (capacity >= 6 ? 3 : 6);
    var best = null;
    var bestSolLen = -1;

    for (var k = 0; k < pool.length && k < maxSolverCalls; k++) {
      if (k > 0 && Date.now() - startTime > GEN_TIME_BUDGET_MS) break;
      var solLen = solverLength(pool[k].bottles, solverBudget);
      var effLen = solLen === null ? Infinity : solLen;
      if (effLen > bestSolLen) { bestSolLen = effLen; best = pool[k].bottles; }
      // Good enough (bar cleared, or provably very hard) - stop searching.
      if (effLen === Infinity || effLen >= requiredSolverMoves) break;
    }

    if (!best) {
      // Should be unreachable (cheapN always tries at least a few boards),
      // but guarantees a legal, playable board rather than throwing.
      best = pool.length ? pool[0].bottles : buildSolvedState(numColors, numEmpty);
    }
    return { bottles: best, numColors: numColors, capacity: capacity };
  }
  ...
  global.WaterSort = {
    ...
    getReverseMoves: getReverseMoves,
    countRuns: countRuns,
  ...
  function levelProgressPct() {
    ...
    state.bottles.forEach(function (stack) {
      if (stack.length === state.capacity) {
  ...
  function buildBottleEl(stack, idx) {
    ...
    for (var pos = 0; pos < state.capacity; pos++) {
